...
app.all("*", (req, res) => {
  ...
  const setResponseStatusCode = parseInt(
    req.headers["x-set-response-status-code"] ||
      req.query["x-set-response-status-code"],
    10
  );
  ...
  const sleepTime = parseInt(
    req.headers["x-set-response-delay-ms"] ||
      req.query["x-set-response-delay-ms"],
    0
  );
  sleep(sleepTime).then(() => {
    //Set the response content type to what the user wants
    const setResponseContentType =
      req.headers["x-set-response-content-type"] ||
    ...
    //Ability to send just the request body in the response, nothing else
      ...
      "response_body_only" in req.query &&
      req.query["response_body_only"] == "true"
    ) {
      res.send(req.body);
    }
...
var httpsServer = https
  .createServer(sslOpts, app)
  .listen(process.env.HTTPS_PORT || 8443);
console.log(
  `Listening on ports ${process.env.HTTP_PORT || 8080} for http, and ${
